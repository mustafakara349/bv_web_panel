@extends('layouts.app')
@section('title', 'Yeni Hizmet Ekle - B&V Barber')
@section('content')
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="fs-3 mb-1">Yeni Hizmet</h1>
                <p class="text-muted">Müşterilerinize sunacağınız yeni bir hizmet kaydedin.</p>
            </div>
                ...
                <form action="{{ route('services.store') }}" method="POST">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="duration_minutes" class="form-control @error('duration_minutes') is-invalid @enderror" value="{{ old('duration_minutes', 30) }}" required min="5">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="gender_type" class="form-select @error('gender_type') is-invalid @enderror" required>
                                <option value="unisex" {{ old('gender_type') == 'unisex' ? 'selected' : '' }}>Unisex</option>
                                <option value="male" {{ old('gender_type') == 'male' ? 'selected' : '' }}>Sadece Erkek</option>
                                <option value="female" {{ old('gender_type') == 'female' ? 'selected' : '' }}>Sadece Kadın</option>
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" name="discounted_price" class="form-control @error('discounted_price') is-invalid @enderror" value="{{ old('discounted_price') }}" min="0">
                        ...
                        <div class="col-12 mt-3">
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="is_popular" id="isPopular" value="1" {{ old('is_popular') ? 'checked' : '' }}>
                    ...
                    <div class="mt-4 text-end">
                        <button type="submit" class="btn btn-primary px-4 rounded-pill shadow-sm">
                            <i class="ti ti-check me-1"></i> Hizmeti Kaydet
                        </button>
                    </div>
                </form>
            </div>
