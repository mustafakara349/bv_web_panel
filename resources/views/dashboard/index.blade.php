            ...
            <div class="card h-100">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <span
                            class="fw-bold">₺{{ number_format($widgets['customers']['avg_spending'] ?? 0, 0, ',', '.') }}</span>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="row g-3 mt-1 mb-3">
        <div class="col-12">
            <div class="card border-warning border-opacity-50 shadow-sm">
                <div class="card-header bg-warning bg-opacity-10 d-flex justify-content-between align-items-center px-4 py-3 border-bottom border-warning border-opacity-25">
                    <h4 class="mb-0 h5 text-warning-emphasis"><i class="ti ti-clock-exclamation me-2"></i>Onay Bekleyen Randevular</h4>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0 border-top-0">
                            <thead class="bg-light">
                                <tr>
                                    <th class="ps-4 text-uppercase text-muted fs-7 fw-semibold py-3 border-0">Tarih / Saat</th>
                                    ...
                                    <th class="text-end pe-4 text-uppercase text-muted fs-7 fw-semibold py-3 border-0">İşlem</th>
                                </tr>
                            </thead>
                            <tbody class="border-top-0">
                                @foreach($pendingAppointments as $apt)
                                    <tr>
                                        <td class="ps-4">
                                            <div class="d-inline-flex align-items-center justify-content-center px-3 py-2 rounded-2 bg-dark text-white fw-bold shadow-sm">
                                                {{ $apt->start_at->format('d.m.Y') }} <i class="ti ti-clock ms-2 me-1 opacity-75"></i> {{ $apt->start_at->format('H:i') }}
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="icon-shape icon-sm bg-primary-subtle text-primary rounded-circle me-2">
                                        ...
                                        <td class="text-end pe-4">
                                            <div class="btn-group shadow-sm">
                                                <form action="{{ route('appointments.update-status', $apt) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="status" value="confirmed">
                                                    <button type="submit" class="btn btn-sm btn-success" title="Onayla" style="border-top-right-radius: 0; border-bottom-right-radius: 0;">
                                                        <i class="ti ti-check me-1"></i>Onayla
                                                    </button>
                                                </form>
                                                <form action="{{ route('appointments.update-status', $apt) }}" method="POST" class="d-inline" onsubmit="return confirm('Bu randevuyu reddetmek istediğinize emin misiniz?');">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="status" value="rejected">
                                                    <button type="submit" class="btn btn-sm btn-danger" title="Reddet" style="border-top-left-radius: 0; border-bottom-left-radius: 0;">
